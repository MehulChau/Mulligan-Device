...
# A per-blob scale outside this band of the median per-blob scale is
# treated as a bad measurement, not real depth change, and replaced with
# the median for that blob alone. Tighter than SIZE_CONSISTENCY_BAND
# on purpose: that band only needs to separate a real ball from something
# else entirely (a distractor off by 10x+); this one is about a single
# noisy diameter estimate among otherwise-legitimate balls, which is a
# smaller effect.
PER_BLOB_SCALE_OUTLIER_BAND = (0.85, 1.15)
...
def _load_calibration(calibration):
    """Normalize `calibration` into (calibration_dict_or_None, source_label).

    Accepts a path to a calibrate.py-produced JSON file, an already-loaded
    calibration dict, or None (no calibration -- the default, so every
    existing synthetic test is unaffected)."""
    if calibration is None:
        return None, None
    if isinstance(calibration, str):
        with open(calibration) as f:
            return json.load(f), calibration
    return calibration, "inline"


def _undistort(image, calibration):
...
def measure(image, strobe_interval_ms, threshold=40, adaptive=False, calibration=None):
    ...
    calibration: optional lens calibration to undistort the frame before
        detection -- a path to a calibrate.py-produced JSON file, or an
        already-loaded calibration dict. None (default) skips
        undistortion entirely, so every existing synthetic test (all
        generated by a distortion-free pinhole model) is unaffected.
    ...
    measurement. calibration_applied/calibration_source do the same for
    whether a measurement can be trusted to be undistorted at all.
    """

    if image.ndim == 3:
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    calibration_data, calibration_source = _load_calibration(calibration)
    if calibration_data is not None:
        image = _undistort(image, calibration_data)
    ...
    # Derive a scale from each blob's extent *perpendicular* to the
    ...
    ordered_diameters = [perp_diameters[i] for i in order]

    mean_diameter_px = float(np.mean(ordered_diameters))
    scale_mm_per_px = BALL_DIAMETER_MM / mean_diameter_px  # representative scale, kept for compatibility

    # The ball doesn't stay at a constant distance from the camera --
    # each blob's own apparent diameter already tells you its own
    # distance, and averaging into one global scale throws that away.
    # Deriving a scale *per blob* and converting each gap with the mean
    # of its two endpoint scales absorbs real depth change for free,
    # instead of blending it into a single wrong-everywhere number.
    per_blob_scales = [BALL_DIAMETER_MM / d for d in ordered_diameters]

    # Guard against a single mis-measured blob: if one blob's scale is
    # wildly out of line with the rest, it's more likely a bad
    # measurement (a noisy diameter estimate) than real depth change --
    # fall back to the median scale for that blob alone and flag it,
    # rather than let one bad value corrupt the two gaps touching it.
    median_scale = float(np.median(per_blob_scales))
    scale_outlier_indices = [
        i for i, s in enumerate(per_blob_scales)
        if not (PER_BLOB_SCALE_OUTLIER_BAND[0] * median_scale <= s <= PER_BLOB_SCALE_OUTLIER_BAND[1] * median_scale)
    ]
    adjusted_scales = [
        median_scale if i in scale_outlier_indices else s for i, s in enumerate(per_blob_scales)
    ]

    spacings_px = np.diff(projections)
    gaps_mm = [
        spacings_px[i] * (adjusted_scales[i] + adjusted_scales[i + 1]) / 2.0
        for i in range(len(spacings_px))
    ]
    ...
    # In real (mm) distance now, not raw pixels, so a genuinely constant-
    # speed flight over changing depth (gaps that are legitimately uneven
    # in *pixels*) doesn't get misread as an uneven-spacing anomaly: the
    # per-gap scale conversion above already absorbs that, leaving this
    # ratio close to 0 for a real constant-speed flight regardless of
    # depth, same as it always was for a constant-depth one.
    spacing_consistency = float(np.std(gaps_mm) / mean_gap_mm) if mean_gap_mm > 0 else float("inf")
    ...
    # width, which the *mean* gap would blend into every interval -- for
    # a 5-flash shot missing one exposure, that's a +33% speed error, not
    # measurement noise. When spacing is this uneven, the *minimum* gap
    # is the right estimate of one genuine flash interval: any gap
    ...
    representative_gap_mm = float(np.min(gaps_mm)) if missing_flash_detected else mean_gap_mm

    speed_mps = (representative_gap_mm / 1000.0) / (strobe_interval_ms / 1000.0)
    ...
    return {
        ...
        "per_blob_scales_mm_per_px": per_blob_scales,
        "scale_spread_mm_per_px": float(np.std(per_blob_scales)),
        "scale_outlier_blobs": len(scale_outlier_indices),
